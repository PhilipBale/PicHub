...
(function ($) {
	...
	***********************************************/
	...
	//pbale
    ImgCache.getImgPath = function (img_src, callback) {
        if (!Private.isImgCacheLoaded()) {
            return;
        }
            ...
            if (ImgCache.options.useDataURI) {
                var _win = function (file) {
                    ...
                    reader.onloadend = function (e) {
                        ...
                        // TODO found file set_path_callback($element, base64content, img_src);
                        Helpers.logging('File ' + filename + ' loaded from cache', LOG_LEVEL_INFO);
                ...
                // using src="filesystem:" kind of url
                var new_url = Helpers.EntryGetURL(entry);
                // todo found file set_path_callback($element, new_url, img_src);
                Helpers.logging('File ' + filename + ' loaded from cache', LOG_LEVEL_INFO);
				callback(new_url);
            }
        };
	...
	ImgCache.cacheFile = function (img_src, img_name, success_callback, fail_callback, on_progress) {
		...
		var filePath = Private.getCachedFileFullPath(img_name);
	...
	// $img: jQuery object of an <img/> element
	ImgCache.useCachedFile = function ($img, $src, success_callback, fail_callback) {

		if (!Private.isImgCacheLoaded()) {
			return;
        }

		Private.loadCachedFile($img, $src, Private.setNewImgPath, success_callback, fail_callback);
	};
